 ...
 * @property null|string $classDbDriverName
    ...
    public function generateActiveField($attribute)
    {
        ...
        if ($tableSchema === false || !isset($tableSchema->columns[$attribute])) {
            if (preg_match('/^(password|pass|passwd|passcode)$/i', $attribute)) {
                return "\$form->field(\$model, '$attribute')->passwordInput(['class'=>'form-control m-input'])";
            }
        ...
        if ($column->phpType === 'boolean') {
            ...
            return "\$form->field(\$model, '$attribute',['options'=>['class'=>'row m-login__form-sub'],'labelOptions' => [ 'class' => 'm-checkbox m-checkbox--focus' ]])->checkbox(['template'=>'<div class=\"col m--align-left\">{beginLabel}{input} {labelTitle}<span></span>{endLabel}{error}{hint}</div>'])";
        }

        if ($column->type === 'text') {
            return "\$form->field(\$model, '$attribute')->textarea(['rows' => 6,'class'=>'form-control m-input'])";
        }
        ...
        if ($column->phpType !== 'string' || $column->size === null) {
            return "\$form->field(\$model, '$attribute')->$input(['class'=>'form-control m-input'])";
        }

        return "\$form->field(\$model, '$attribute')->$input(['maxlength' => true,'class'=>'form-control m-input'])";
    }
